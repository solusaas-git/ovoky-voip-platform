...
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/')) {
    ...
    if (request.method === 'OPTIONS') {
      return new NextResponse(null, {
        status: 200,
        headers: {
          'Access-Control-Allow-Origin': process.env.NEXT_PUBLIC_APP_URL || 'https://app.ovoky.io',
  ...
  if (!authToken && !isPublicPath) {
    ...
    if (pathname.startsWith('/api/')) {
      ...
      response.headers.set('Access-Control-Allow-Origin', process.env.NEXT_PUBLIC_APP_URL || 'https://yourdomain.com');
  ...
  if (authToken && (pathname === '/' || pathname === '/register')) {
