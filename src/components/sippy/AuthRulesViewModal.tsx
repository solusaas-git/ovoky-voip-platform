...
export function AuthRulesViewModal({ isOpen, onClose, accountId }: AuthRulesViewModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!remoteIp.trim()) {
      setError('Remote IP address is required');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to create authentication rule');
      }
      ...
      setError(err instanceof Error ? err.message : 'Failed to create authentication rule');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <DialogContent className="max-w-[60vw] sm:max-w-[60vw] w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-auto">
          ...
          {showForm && (
            <Card className="mb-4">
              <CardHeader>
                <CardTitle className="text-lg">Add Authentication Rule</CardTitle>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Enter the IP address or CIDR range that should be allowed to authenticate. 
                      Other settings will use secure defaults (Protocol: SIP, Sessions: Unlimited, CPS: Unlimited).
                    </p>
                  </div>
